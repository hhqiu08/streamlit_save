#streamlit run d:\大模型\streamlit\vis\get_vis.py [ARGUMENTS]
import streamlit as st 
import pandas as pd
import numpy as np


D = pd.read_excel('D:/大模型/streamlit/vis/result.xlsx')
D2 = pd.read_excel('D:/大模型/streamlit/vis/save_data.xlsx')
def read(data):
    l=[]
    for i in range(1,6):
...
if app_mode == '各项指标展示':
    st.header('指标展示')
    st.image('D:/大模型/streamlit/vis/图/线条小狗2.jpg')     
    ...
    if score == "0~10":
        st.image('D:/大模型/streamlit/vis/图/线条小狗1.jpg')    
        st.subheader('0~10分:') 
        st.markdown('数据 :')       
        st.write(D)
        st.markdown('综合评分图示')    		 
        ...
        st.image('D:/大模型/streamlit/vis/图/2.png')
        st.subheader('0~3分:') 
        page2(D.iloc[381:,[0,-1]],D.iloc[381:,:-1],l[4][:4])
        
    elif score == "4~6":
        st.image('D:/大模型/streamlit/vis/图/1.png')
        st.subheader('4~6分:') 
        page2(D.iloc[13:381,[0,-1]],D.iloc[13:381,:-1],l[4][4:7])
    else:
        st.image('D:/大模型/streamlit/vis/图/3.png')
        st.subheader('7~10分:') 
